...
function check_compile() {
  # $1: filter
  folder_board_v2015="board/khadas/defconfigs"
  customer_folder_v2015="customer/board/defconfigs"
  ...
  then
    filter=$1
    for file in ${folder_board_v2015}/*; do
	  ...
	  if [ -n "$temp_file" ] && [[ -e ${folder_board_v2015}/${temp_file}'_defconfig' ]]; then
  ...
  then
    filter=""
    if [[ -e ${customer_folder_v2015} ]]; then
      for file in ${customer_folder_v2015}/*; do
        temp_file=`basename $file`
        temp_file=${temp_file%_*}
        #echo $temp_file
